/* src/ops/mul.c — Multiplicación elemento a elemento con retropropagación */
...
static void _bwd_multiplicar(_nodo_autograd* n) {
    ...
    if (n->entradas[0]->requiere_grad) {
        ...
        for (size_t i=0;i<sz;i++) tmp[i]=g[i]*db[i];
        _acumular_grad(n->entradas[0],tmp);
        free(tmp);
    }
    if (n->entradas[1]->requiere_grad) {
        float* tmp=(float*)malloc(sz*sizeof(float));
        for (size_t i=0;i<sz;i++) tmp[i]=g[i]*da[i];
...
tc_tensor* tc_multiplicar(tc_tensor* a, tc_tensor* b) {
    ...
    if (s->requiere_grad) _tape_empujar(s,a,b,2,0,NULL,_bwd_multiplicar);
    return s;
}
